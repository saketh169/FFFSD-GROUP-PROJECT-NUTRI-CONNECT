...
const publicRoutes = ["/", "/blog", "/contact", "/role_signin", "/role_signup", "/post", 
  "/submit", "/blog/:id" ,"/Sign_in","/Sign_up" , "/chatbot" ,"/privacy-policy","/terms_conditions"];
...
router.get("/blog", (req, res) => {
  res.render("blog", { blogs: blogs }); // Render the blog.ejs file with blogs data
});
...
router.get("/role_signin", (req, res) => {
  res.render("roles_signin"); // Render the roles_signin.ejs file
});

// Role sign-up page
router.get("/role_signup", (req, res) => {
  res.render("roles_signup"); // Render the roles_signup.ejs file
});
...
router.post("/submit", upload.single("image"), (req, res) => {
  ...
  if (req.file) {
    ...
    image = `/blogs/${req.file.filename}`; // Path to the uploaded file
  } else if (imageUrl) {
...
router.get("/blog/:id", (req, res) => {
  ...
  const blog = blogs[blogId]; // Fetch the blog from the array

  if (!blog) {
    return res.status(404).render("404", { message: "Blog not found" }); // Render 404 if blog doesn't exist
  }
...
router.get("/pricing", ensureAuthorized("user"), (req, res) => {
  res.render("pricing"); // Render pricing.ejs
});
