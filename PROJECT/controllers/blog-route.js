...
// Route to handle homepage redirection based on session data
router.get('/home', (req, res) => {
  ...
  if (req.session) {
    ...
    // Check for dietitian session
    if (req.session.dietitian) {
       return res.redirect('/dietitian');
      
    }

    // Check for organization session
  ...
  // Fallback: redirect to general homepage if no valid session or user type
  return res.redirect('/');
});
